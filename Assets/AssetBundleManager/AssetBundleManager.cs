...
{
	/// <summary>
	/// Loaded assetBundle contains the references count which can be used to unload dependent
	/// assetBundles automatically.
	/// </summary>
	...
	public class AssetBundleManager : MonoBehaviour
	{
		...
		/// <summary>
		/// The base downloading url which is used to generate the full downloading url with the
		/// assetBundle names.
		/// </summary>
		...
		/// <summary>
		/// Implements per-bundle base downloading URL override. The subscribers must return null
		/// values for unknown bundle names;
		/// </summary>
		...
		/// <summary>
		/// AssetBundleManifest object which can be used to load the dependecies and check suitable
		...
		/// the web-server should have the same structure as the AssetBundles directory in the demo
		/// project root. </summary> <example>For example, AssetBundles/iOS/xyz-scene must map to
		/// absolutePath/iOS/xyz-scene. <example>
		...
		/// <summary>
		/// Retrieves an asset bundle that has previously been requested via LoadAssetBundle. Returns
		/// null if the asset bundle or one of its dependencies have not been downloaded yet.
		/// </summary>
		...
		/// <summary>
		/// Returns true if certain asset bundle has been downloaded without checking whether the
		...
		/// <summary>
		/// Initializes asset bundle namager and starts download of manifest asset bundle. Returns
		/// the manifest asset bundle downolad operation object.
		/// </summary>
		...
		// Returns base downloading URL for the given asset bundle. This URL may be overridden on
		// per-bundle basis via overrideBaseDownloadingURL event.
		...
		//
		// On most platforms, this is done by the AssetBundleManager itself. However, on certain
		// platforms (iOS at the moment) it's possible that an external asset bundle variant
		// resolution mechanism is used. In these cases, we use base asset bundle name (without the
		// variant tag) as the bundle identifier. The platform-specific code is responsible for
		// correctly loading the bundle.
		...
		static protected bool LoadAssetBundleInternal(string assetBundleName, bool isLoadingAssetBundleManifest)
		{
			...
			// @TODO: Do we need to consider the referenced count of WWWs? In the demo, we never have
			// duplicate WWWs as we wait LoadAssetAsync()/LoadLevelAsync() to be finished before
			// calling another LoadAssetAsync()/LoadLevelAsync(). But in the real case, users can
			// call LoadAssetAsync()/LoadLevelAsync() several times then wait them to be finished
			// which might have duplicate WWWs.
